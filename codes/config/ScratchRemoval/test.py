...
####ScratchDetection
orPath = 'datasets\\rain\\testH\\LQ'
#orPath = 'datasets/rain/testH/LQ'

print("ScratchDetection")
print(orPath)
mask_dir = os.path.join("masks")
new_input = os.path.join(mask_dir, "input")
new_mask = os.path.join(mask_dir, "mask")
final_scratch = os.path.join(mask_dir, "final")
...
##建立生成的blur和分割出划痕内容的存放文件夹
if not os.path.exists('blur'):
    os.mkdir('blur')
if not os.path.exists('contentMask'):
    os.mkdir('contentMask')
    presentMaskCopy = Image.new('RGB', (100, 100), (0, 0, 0)) ####初始化
    presentMaskCopy = np.asarray(presentMaskCopy)
    cv2.imwrite("contentMask\\contentMask.png", presentMaskCopy)
    #cv2.imwrite("contentMask/contentMask.png", presentMaskCopy)
...
for test_loader in test_loaders:
    ...
    for i, test_data in enumerate(test_loader):
        ...
        orPath = 'datasets\\rain\\testH\\LQ'
        #orPath = 'datasets/rain/testH/LQ'
        orImgName = img_name + ".png"
        print(orImgName)
        orImg = os.path.join(orPath, orImgName)
        orImg = cv2.imread(orImg)
        blurImg = cv2.GaussianBlur(orImg, (101, 101), 300)
        cv2.imwrite("blur\\blur.png", blurImg)
        #cv2.imwrite("blur/blur.png", blurImg)
        ...
        presentMask = img_name + ".png"
        print(orImgName)
        presentMaskPath = os.path.join(final_scratch, presentMask)
        presentMaskCopy = cv2.imread(presentMaskPath,-1)
        #cv2.imwrite("contentMask\\contentMask.png", presentMaskCopy)
        cv2.imwrite("contentMask/contentMask.png", presentMaskCopy)
