...
class IRSDE(SDE):
    ...
    def _initialize(self, max_sigma, T, schedule, eps=0.01):
        ...
        img_mask2 = cv2.imread('contentMask\\contentMask.png',-1)
        #img_mask2 = cv2.imread('contentMask/contentMask.png', -1)
        img_mask2 = img2tensor(img_mask2)
        self.img_mask2 = img_mask2.to(self.device)


        img_encode = cv2.imread('contentMask\\contentMask.png', -1)
        #img_encode = cv2.imread('contentMask/contentMask.png', -1)
        height, width, _ = img_encode.shape

        transf = transforms.ToTensor()
        img_tensor = transf(img_encode)
        ...
        class Encoder(torch.nn.Module):
            ...
            def __init__(self, input_size, hidden_size, latent_size):
                ...
                self.muE = torch.nn.Linear(hidden_size, latent_size)
                self.sigmaE = torch.nn.Linear(hidden_size, latent_size)

            def forward(self, x):  # x: bs,input_size
                x = F.relu(self.linear(x))  # -> bs,hidden_size
                muE = self.muE(x)  # -> bs,latent_size
                sigmaE = self.sigmaE(x)  # -> bs,latent_size
                return muE, sigmaE

        bs = img_tensor.shape[0]
        # 获取数据
        imgss = img_tensor.view(bs, width * height)  # imgs:(bs,28*28)

        encoder = Encoder(width * height, 128, 3)
        muE, sigmaE = encoder(imgss)

       
        '''eps = torch.randn(size=(mu.shape))
        std = torch.sqrt(torch.exp(sigma))
        z = mu + eps * std'''
        ...
        mu2 = muE[0][1]
        sigma3 = sigmaE[0][1]
        sigma3 = math.exp(sigma3)

        mu3 = muE[0][2]
        sigma4 = sigmaE[0][2]
        sigma4 = math.exp(sigma4)
        self.sigma2 = sigma2
        self.sigma3 = sigma3
        self.sigma4 = sigma4
        self.mu1 = mu1
    ...
    def sde_reverse_drift(self, x, score, t):
        img_mask = cv2.imread('blur\\blur.png',-1)
        #img_mask = cv2.imread('blur/blur.png', -1)
        img_mask = img2tensor(img_mask)
        #img_mask = img_mask.unsqueeze(0)
        self.img_mask = img_mask.to(self.device)
        self.img_mask = self.mu.type(torch.cuda.LongTensor)
        self.img_mask2 = self.mu.type(torch.cuda.LongTensor)

        return (self.thetas[t] * (self.mu - x - self.img_mask + self.img_mask2) - self.sigmas[t] ** 2 * score) * self.dt
    def ode_reverse_drift(self, x, score, t):
    ...
    def get_score_from_noise2(self, noise):
        return -noise / self.sigma2
    def get_score_from_noise3(self, noise):
        return -noise / self.sigma3
    def get_score_from_noise4(self, noise):
        return -noise / self.sigma4

    def score_fn(self, x, t):
        # need to pre-set mu and score_model
        #self.img = self.mu.type(torch.cuda.LongTensor)
        noise = self.model(x, self.mu, t)
        return self.get_score_from_noise(noise, t)

    def score_fn2(self, x,t):
        # need to pre-set mu and score_model
        #self.mu1 = self.mu.type(torch.cuda.LongTensor)
        noise = self.model(x, self.img, t)
        return self.get_score_from_noise2(noise)
    def score_fn3(self, x, t):
        # need to pre-set mu and score_model
        #self.mu2 = self.mu.type(torch.cuda.LongTensor)


        noise = self.model(x, self.img, t)
        return self.get_score_from_noise3(noise)
    def score_fn4(self, x, t):
        # need to pre-set mu and score_model
        #self.mu3 = self.mu.type(torch.cuda.LongTensor)


        noise = self.model(x, self.img, t)
        return self.get_score_from_noise4(noise)
    ...
    def reverse_sde(self, xt, T=-1, save_states=False, save_dir='sde_state'):
        ...
        for t in tqdm(reversed(range(1, T + 1))):
            ...
            score = self.score_fn(x, t)
            score2 = self.score_fn2(x, t)
            score3 = self.score_fn3(x,t)
            score4 = self.score_fn4(x, t)
            if t >= 50:
                x = self.reverse_sde_step(x, 0.9 * score + 0.09 * score2 + 0.09 * score3 + 0.09 * score4, t)
                #x = self.reverse_sde_step(x, score, t)
            else:
                x = self.reverse_sde_step(x, score, t)
            if save_states:  # only consider to save 100 images
                ...
                if t % interval == 0:
                    idx = t // interval
                    os.makedirs(save_dir, exist_ok=True)
                    tvutils.save_image(x.data, f'{save_dir}/state_this{idx}.png', normalize=False)
